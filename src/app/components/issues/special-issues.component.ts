...
@Component({
  selector: 'sissues',
  templateUrl: './special-issues.component.html',
  imports: [CommonModule, ReactiveFormsModule, HttpClientModule],
})
export class SpecialIssuesComponent {
  ...
  private initializeData(): void {
    this.issueService.getIssues('special').subscribe({
      next: (data) => {
        this.allIssues = data;
    ...
    this.filteredSpecialIssues = this.allIssues;
  }

  applyFilters(filters: any) {
    this.filteredSpecialIssues = this.allIssues.filter((issue) => {
  ...
  sortIssues() {
    this.filteredSpecialIssues.sort((a, b) => {
  ...
  updatePagination() {
    this.totalPages = Math.ceil(
      this.filteredSpecialIssues.length / this.pageSize
    );
    const startIndex = (this.currentPage - 1) * this.pageSize;
    this.paginatedIssues = this.filteredSpecialIssues.slice(
      startIndex,
  ...
  public downloadIssue(id: number): void {
    ...
    if (issue) {
      ...
      this.issueService.updateDownloadCount(id, 'special');
    }
  }
